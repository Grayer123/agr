...
public class MyCircularQueue {
    ...
    public bool EnQueue(int value) { 
        if(count == circularArr.Length) { // the circular queue is full
            return false; // cannot insert
        }
        rear = (rear + 1) % circularArr.Length; // rear points to the last elem of arr, insert new elem, then rear++
    ...
    public bool DeQueue() {
        if(count == 0) { // the curcular queue is empty
            return false; 
        }
    ...
    private int front;  //point to the first elem
    private int rear; // point to the tail elem
    private int count; // num of elems in the circluar queue
}
