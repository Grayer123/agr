...
public class Solution {
    public bool WordPattern(string pattern, string str) {
        ...
        Dictionary<string, char> dictStr = new Dictionary<string, char>();
        ...
        for(int i = 0; i < pattern.Length; i++) {
            ...
            if(!dictStr.ContainsKey(strArr[i])) {
                dictStr[strArr[i]] = pattern[i];
            }
            if(dictPat[pattern[i]] != strArr[i] || dictStr[strArr[i]] != pattern[i]) {
                return false;
            }
        }
        return true;
